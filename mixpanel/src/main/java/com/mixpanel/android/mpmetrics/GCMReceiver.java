...
//        final Intent appIntent = manager.getLaunchIntentForPackage(context.getPackageName());
        Intent appIntent = new Intent(context.getPackageName()+".activity.DialogMessage");
        appIntent.putExtra("mp_message",message);
        ...
        if (Build.VERSION.SDK_INT < 11) {
            ...
            showNotificationSDK17OrHigher(context, contentIntent, notificationIcon, notificationTitle, message);
        }
    }
    ...
    @TargetApi(16)
    private void showNotificationSDK17OrHigher(Context context, PendingIntent intent, int notificationIcon, CharSequence title, CharSequence message){
        final NotificationManager nm = (NotificationManager)context.getSystemService(Context.NOTIFICATION_SERVICE);
